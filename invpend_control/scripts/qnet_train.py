...
class QlearnCartPole(CartPole):
    ...
    def train(self):
        # Initialize learning
        input_state_node = tf.placeholder(tf.float32, shape=(1,4))
        gamma = .99
        epsilon = .1
        # create oist to store total rewards and steps taken per epoch
        with tf.Session() as sess:
            ...
            for epoch in range(num_epochs):
                ...
                while step < MAX_STEP and not out:
                    action_index, q_value = sess.run([predict, q_output], feed_dict={input_state_node:np.array(ob)})
